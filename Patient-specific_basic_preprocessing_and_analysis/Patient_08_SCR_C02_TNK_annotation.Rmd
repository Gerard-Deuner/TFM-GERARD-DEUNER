---
title: "Patient_08_SCR_C02_TNK_annotation"
...
data.tnk.2 <- data.tnk %>%
  ...
  ScaleData(vars.to.regress = c("nFeature_RNA", "percent.mt", "nCount_RNA")) %>% #improves integration
  RunPCA()
```
...
```{r}
data.tnk.2 <- RunHarmony(data.tnk.2, group.by.vars = "project") # project
data.tnk.2 <- data.tnk.2 %>%
...
data.tnk$sample[data.tnk$sample_id == "SPE_1_8_C2_A_FRESH_1"] <- "C02.1"
data.tnk$sample[data.tnk$sample_id == "SPE_1_8_C2_A_FRESH_2"] <- "C02.2"
data.tnk$sample[data.tnk$sample_id == "SPE_1_08_SCR_A_FRESH"] <- "SCR"
```

```{r}
# check for batch effects - check correction
p1 <- DimPlot_scCustom(data.tnk.2, reduction = "pca", group.by = "project") + NoLegend() + 
DimPlot_scCustom(data.tnk.2, reduction = "harmony", group.by = "project")
p1
```
...
int.plot <- DimPlot_scCustom(data.tnk.2, group.by = "sample", colors_use = sample.colors, figure_plot = T, )
int.plot
```
...
data.tnk <- data.tnk %>% 
  ...
  AddMetaData(metadata = Idents(data.tnk), col.name = paste0("cluster_res", "1.3"))

cl <- DimPlot_scCustom(data.tnk, group.by = "cluster_res1.3", label = T, repel = T, figure_plot = T)
cl
```

```{r}
ggsave(filename = paste0(fig.dir, patient, "_TNK_subset_SCR_CO2_clustering_res1.3.tiff"), plot = cl, device = "tiff", width = 7, height = 6, dpi = 600)
```

```{r}
data.tnk <- SetIdent(data.tnk, value = "cluster_res1.3")
...
markers %>%
  group_by(cluster_res1.3) %>%
  slice_max(n = 20, order_by = avg_log2FC)
```

```{r}
# save markers
write.csv(markers, file = paste0(data.dir, "markers/", patient, "/", patient, "_TNK_subset_res1.3_markers.csv"))
```

```{r}
# read markers
markers <- read.csv(file = paste0(data.dir, "markers/", patient, "/", patient, "_TNK_subset_res1.3_markers.csv"))
```
...
openxlsx::write.xlsx(markers_split, 
                     file = paste0(data.dir, "markers/", patient, "/", patient, "_TNK_subset_res1.3_cluster_markers.xlsx"),
                     overwrite = TRUE)
```
...
annotation <- c(      "CD4+ T Naïve",            #0 
                      "CD56bright CD16lo NK",    #1 no CD16
                      "CD8+ Trm Exhausted",               #2 
                      "CD4+ T Central Memory",          #3 FOS JUN TNF -> Activation
                      "CD56dim CD16hi NK",                #4
                      "CD8+ T Effector",                            #5
                      "MAIT",                        #6 
                      "CD8+ T Effector Memory",                              #7 
                      "CD8+ T Effector Memory",                     #8 
                      "CD4+ T Regulatory",                  #9 
                      "CD8+ Trm Exhausted",                #10 CD4+/CD8+ Trm Exhaustion
                      "T Proliferative",                  #11 
                      "CD8+ T Naïve",                             #12 
                      "CD4+ T Follicular Helper",         #13
                      "Cycling T",                 #14
                      "NKT",                 #15 #doublet score very high. not clear cell type. TRDC
                      "NOISE",                     #16 no CD4 no CD8
                      "IFN T Responding",                 #17
                      "NOISE"                 #18 dbl score high, high mt, high ngenes ... 
...
# number of cells
Cells(data.tnk.cl) %>% length #10411
```
...
anno2.1 <- DimPlot_scCustom(data.tnk.cl, 
                            ...
                            repel = T, 
                            figure_plot = T,
                            colors_use = DiscretePalette_scCustomize(num_colors = 16, palette = "varibow")
                            ) +
  labs(x = "UMAP 1", y = "UMAP 2")
anno2.1
```
