---
title: "Patient_03_SCR_C02_myeloid_annotation"
...
anno <- DimPlot_scCustom(data, 
                         ...
                         label = F, 
                         colors_use = c("red", "blue", "darkorange", "darkgreen"),
                         figure_plot = T) 
anno
```
...
data.m$sample[data.m$sample_id == "SPE_1_03_C02_A_FRESH_C45"] <- "C02"
data.m$sample[data.m$sample_id == "SPE_1_03_SCR_A_FRESH"] <- "SCR"
```
...
```{r}
annotation <-c("Angio TAM",               #0 
               "Neutrophil-like",         #1 low num features and counts
               "CD14+ CD16- Mono",        #2
               "LA TAM",                  #3 
               "NOISE",                   #4 high MT #subclustering
               "Anti Inflam TAM",         #5
               "IFN TAM",                 #6 
               "NOISE",                   #7 very weird genes
               "IFN TAM",                 #8
               "NOISE",                   #9  T cell doublets
               "NOISE",                   #10 pDC doublets #subclustering
               "Neutrophil-like"          #11 
)
...
```{r}
# Subcluster clusters 4 and 10
data.m <- SetIdent(data.m, value = "cluster_res1.3") %>%
  FindSubCluster(cluster = c("10"), 
                 graph.name = "RNA_snn", 
                 resolution = 1, 
                 subcluster.name = "cluster_res1.3.1") %>%
  SetIdent(value = "cluster_res1.3.1") %>% 
  FindSubCluster(cluster = c("4"), 
                 graph.name = "RNA_snn", 
                 resolution = 1, 
                 subcluster.name = "cluster_res1.3.1")

data.m$cluster_res1.3.1 <- as.factor(data.m$cluster_res1.3.1)

cl2 <- DimPlot(data.m, group.by = "cluster_res1.3.1")
cl2
```

```{r}
# find markers of subclustering
data.m <- SetIdent(data.m, value = "cluster_res1.3.1")
markers <- FindAllMarkers(data.m, only.pos = T, min.pct = 0.25)

# save markers
write.csv(markers, file = paste0(sp.data.dir, "/", "markers/", patient, "/", patient, "_myeloid_subset_res1.3.1_markers.csv"))
...
openxlsx::write.xlsx(markers_split, 
                     file = file.path(sp.data.dir, "markers", patient, paste0(patient, "_myeloid_subset_res1.3.1_cluster_markers.xlsx")),
                     overwrite = TRUE)
```
...
annotation <-c("Angio TAM",               #0 
               ...
               "CD14+ CD16- Mono",        #2
               "LA TAM",                  #3 
               "NOISE",                   #4_0 still NOISE
               "NOISE",                   #4_1 still NOISE
               "NOISE",                   #4_2 still NOISE
               "Anti Inflam TAM",         #5
               "IFN TAM",                 #6 
               "NOISE",                   #7 very weird genes
               "IFN TAM",                 #8
