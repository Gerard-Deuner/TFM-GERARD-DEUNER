---
title: "Patient_10_Tumor-Blood_TCR_Analysis"
author: "Gerard Deuner Cos"
date: "2024-05-07"
output: html_document
---

# Patient 10 Tumor & Tumor-Blood Matching TCR Preprocessing & Analysis
...
data <- qread("/Users/gerarddeunercos/Documents/SERPENTINE/data/outputdata/Patient_10_SCR_CO2_TNK_subset_pp_annotated_clean_30-01-24")
```
...
contig_list_TCR <- list(P10_S2, P10_S4)
...
combined_tcr <- combineTCR(contig_list_TCR, 
                ID = c(
                  "SPE_1_10_SCR_A_FRESH_CD45pos", "SPE_1_10_C2_A_FRESH_CD45pos"
                  ), 
                samples = c(
                  "SERPENTINE_28_29", "SERPENTINE_36_37"
                  )
                )
...
data$blood_pre <- ifelse(data$CTnt_TRB %in% p10_pre_blood_tcr$cdr3_nt, T, F)
data$blood_post <- ifelse(data$CTnt_TRB %in% p10_post_blood_tcr$cdr3_nt, T, F)
...
write.csv(p10.pre %>% distinct(), file = "/Users/gerarddeunercos/Documents/cluster1/gdeuner/SERPENTINE/data/TCR_tumor_blood_analysis/Patient_10/files/SPE10_tumour_overlap_blood_pre_phenotype.csv")

# post
p10.post <- data@meta.data %>%
  select("CTnt", "CTnt_TRB", "cell_type", "Annotation_2.0", "blood_post") %>%
  filter(blood_post == T) %>%
  select("CTnt_TRB", "cell_type", "Annotation_2.0") %>%
  dplyr::rename(CTnt = CTnt_TRB, CellType_1_0 = cell_type, CellType_2_0 = Annotation_2.0)
write.csv(p10.post %>% distinct(), file = "/Users/gerarddeunercos/Documents/cluster1/gdeuner/SERPENTINE/data/TCR_tumor_blood_analysis/Patient_10/files/SPE10_tumour_overlap_blood_post_phenotype.csv")
``` 
...
tcr.overlap <- read.csv("/Users/gerarddeunercos/Documents/cluster1/gdeuner/SERPENTINE/data/differential_expansion_overlap_tumour/SPE10_tumour_blood_differential_overlap.csv")
```
...
write.csv(p10.diffExp %>% distinct(), file = "/Users/gerarddeunercos/Documents/cluster1/gdeuner/SERPENTINE/data/TCR_tumor_blood_analysis/Patient_10/files/SPE10               _tumour_blood_differential_overlap_phenotype.csv")
```

```{r}
# quality control
p10.diffExp$CTnt %>% length()
p10.diffExp$CTnt %>% unique() %>% length()
intersect(p10.diffExp$CTnt, tcr.overlap$cdr3_nt) %>% length()
```
...
qsave(data, "/Users/gerarddeunercos/Downloads/P10_TNK_TCR_updated_07-05-24.qs")
```
...
SaveH5Seurat(data, filename = "/Users/gerarddeunercos/Downloads/P10_TNK_TCR_updated_07-05-24", dest = "h5seurat", overwrite = T)
Convert(file.path("/Users/gerarddeunercos/Downloads/P10_TNK_TCR_updated_07-05-24.h5seurat"), dest = "h5ad", overwrite = T)
```
