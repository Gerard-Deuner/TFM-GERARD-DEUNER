---
title: "Patient_01_SCR_C02_overview"
...
data <- data %>%
  ...
  ScaleData() %>%
  RunPCA()
data <- RunHarmony(data, group.by.vars = "sample") # by sample as replicates show some batch 
data <- data %>%
...
int.plot <- DimPlot(data, group.by = "sample") + 
  scale_color_manual(labels = c("CO2.1", "C02.2", "SCR.1", "SCR.2.CD45"), values=sample.colors)
int.plot
```
...
anno <- DimPlot(data, group.by = "Annotation_1.0", label = F) + 
  scale_color_manual(values = c("red", "blue", "darkorange","darkgreen",  "green", "skyblue", "gold")) + 
...
marker.labels.2 <- c("Naïve", "Activated", "Exhaustion", "Proliferation", "Tregs")
...
```{r eval = FALSE}
S1 <- read.csv(paste0(proj.dir, "data/SERPENTINE_04_05/jobs/01_CD45/01_CD45/outs/per_sample_outs/01_CD45/vdj_t/filtered_contig_annotations.csv"))
S2 <- read.csv(paste0(proj.dir, "data/SERPENTINE_04_05/jobs/01_total/01_total/outs/per_sample_outs/01_total/vdj_t/filtered_contig_annotations.csv"))
S3<- read.csv(paste0(proj.dir,"data/SERPENTINE_08_09/jobs/SPE_1_01_C2D1_A_FRESH_1/SPE_1_01_C2D1_A_FRESH_1/outs/per_sample_outs/SPE_1_01_C2D1_A_FRESH_1/vdj_t/filtered_contig_annotations.csv"))
S4 <- read.csv(paste0(proj.dir,"data/SERPENTINE_08_09/jobs/SPE_1_01_C2D1_A_FRESH_2/SPE_1_01_C2D1_A_FRESH_2/outs/per_sample_outs/SPE_1_01_C2D1_A_FRESH_2/vdj_t/filtered_contig_annotations.csv"))

contig_list_TCR <- list(S1, S2, S3, S4)

# TCR
combined_tcr <- combineTCR(contig_list_TCR, 
                ID = c("01_CD45", "01_total", "SPE_1_01_C2D1_A_FRESH_1", "SPE_1_01_C2D1_A_FRESH_2"), 
                samples = c("SERPENTINE_04_05", "SERPENTINE_04_05", "SERPENTINE_08_09", "SERPENTINE_08_09")
                )
...
```
```{r}
# genes <- c("PTPRC", "CD4", "CD8B", "FOXP3", "HAVCR2", "MKI67")
# plots <- c()
# for (gene in genes){
#   print(gene)
#   plot <- FeaturePlot_scCustom(data, features = gene) + 
